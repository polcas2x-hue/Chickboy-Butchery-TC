// Port of Code.gs's deleteScheduleEntry (Code.gs:4597). Super Admin only,
// hard delete -- no soft-delete/archive in the original.
import { createClient } from 'jsr:@supabase/supabase-js@2';
import { handlePreflight, jsonResponse } from '../_shared/cors.ts';
import { requireRole, AuthError } from '../_shared/session.ts';

Deno.serve(async (req) => {
  const preflight = handlePreflight(req);
  if (preflight) return preflight;

  let body: { token?: string; id?: string };
  try {
    body = await req.json();
  } catch {
    return jsonResponse({ success: false, message: 'Invalid request.' }, 400);
  }

  const supabaseAdmin = createClient(Deno.env.get('SUPABASE_URL')!, Deno.env.get('SUPABASE_SERVICE_ROLE_KEY')!);

  try {
    await requireRole(supabaseAdmin, body.token, ['Super Admin']);
  } catch (err) {
    if (err instanceof AuthError) return jsonResponse({ success: false, message: err.message }, err.status);
    throw err;
  }

  if (!body.id) {
    return jsonResponse({ success: false, message: 'Missing schedule ID.' });
  }

  const { data, error } = await supabaseAdmin.from('training_schedule').delete().eq('id', body.id).select('id').maybeSingle();
  if (error) {
    return jsonResponse({ success: false, message: `Could not delete schedule: ${error.message}` }, 500);
  }
  if (!data) {
    return jsonResponse({ success: false, message: 'Schedule entry not found.' });
  }

  return jsonResponse({ success: true, message: 'Training schedule deleted.' });
});
